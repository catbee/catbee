  ...
  lab.experiment('#route', function () {
    ...
    lab.beforeEach(function (done) {
      ...
      var documentRenderer = {
        ...
        updateState: function (context, options) {
          eventBus.emit('stateUpdated', context, options);
          return Promise.resolve();
        }
      };
    ...
    lab.test('should catch internal link click and change state', function (done) {
      ...
      eventBus.once('documentRendered', function (context) {
        ...
        assert.strictEqual(context.args.third, 'hello');
        assert.strictEqual(context.location.toString(), 'http://local/some');
        isChecked = true;
      });

      eventBus.once('stateUpdated', function (context) {
        assert.strictEqual(context.args.first, 'firstValue');
        assert.strictEqual(context.args.second, 'secondValue');
        assert.strictEqual(context.args.third, 'hello');
        assert.strictEqual(context.args.global, 'globalValue');
      });
    ...
    lab.test('should catch click on item inside link and change state', { skip: true }, function (done) {
      var documentRenderer = locator.resolve('documentRenderer');
      var eventBus = locator.resolve('eventBus');
      var isChecked = false;
      var link = '/some/?global=globalValue&first=firstValue&second=secondValue';
    ...
    lab.test('should catch link click and change state if link starts with //', { skip: true }, function (done) {
    ...
    lab.test('should properly handle relative URIs with .. and change state', { skip: true }, function (done) {
      var locator = createLocator();
      var documentRenderer = locator.resolve('documentRenderer');
      var eventBus = locator.resolve('eventBus');
      var isChecked = false;
    ...
    lab.test('should not change state if link changes host', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
    ...
    lab.test('should not change state if link has "target" attribute', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
      var link = 'http://local1.com/some/?global=globalValue&first=firstValue&second=secondValue';
    ...
    lab.test('should not change state if click on element that is not inside the link', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
      var link = 'http://local1.com/some/?global=globalValue&first=firstValue&second=secondValue';
    ...
    lab.test('should not change state if link has been clicked by middle mouse button', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
      var link = 'http://local1.com/some/?global=globalValue&first=firstValue&second=secondValue';
    ...
    lab.test('should not change state if link has been clicked with Control', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
      var link = 'http://local1.com/some/?global=globalValue&first=firstValue&second=secondValue';
    ...
    lab.test('should not change state if link has been clicked with Alt', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
      var link = 'http://local1.com/some/?global=globalValue&first=firstValue&second=secondValue';
    ...
    lab.test('should not change state if link has been clicked with Shift', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
      var link = 'http://local1.com/some/?global=globalValue&first=firstValue&second=secondValue';
    ...
    lab.test('should not change state if link does not have "href" attribute', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
    ...
    lab.test('should not run signal if invoked with silent flag', { skip: true }, function (done) {
      var locator = createLocator();
      var eventBus = locator.resolve('eventBus');
      var documentRenderer = locator.resolve('documentRenderer');
