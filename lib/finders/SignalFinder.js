...
class SignalFinder extends EventEmitter {
  ...
  watch () {
    ...
    this._fileWatcher = chokidar.watch([this._signalsFolder, this._actionsFolder], CHOKIDAR_OPTIONS)
      .on('add', this._reload.bind(this))
      .on('change', this._reload.bind(this))
      .on('unlink', this._reload.bind(this));
  }
   ...
   */
  _reload () {
    var reloadSignals = new Promise((fulfill, reject) => {
      this._foundSignals = null;
      var signalsFilesGlob = new glob.Glob(this._signalsGlobExpression, GLOB_SETTINGS);

      signalsFilesGlob
        .on('match', match => {
          requireHelper.clearCacheKey(requireHelper.getAbsoluteRequirePath(match));
        })
        .on('error', () => {
          reject();
        })
    ...
    var reloadActions = new Promise((fulfill, reject) => {
