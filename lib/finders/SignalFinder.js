...
class SignalFinder extends EventEmitter {
  ...
  constructor ($eventBus, signalsFolder, actionsFolder) {
    ...
    this._actionsFolders = actionsFolder ? actionsFolder.split(',') : [ACTIONS_DEFAULT_FOLDER];
    this._actionsGlobExpressions = this._actionsFolders
      .map(folder => path.join(folder, ACTIONS_DEFAULT_GLOB))
      .map(expression => requireHelper.getValidPath(expression));
  }
  ...
  _reload () {
    ...
    return Promise.all([
        ...
        ...this._actionsGlobExpressions
          .map(expression => this._reloadByGlob(expression))
      ])
