...
class SignalFactory {
  ...
  create (name, chain) {
    ...
    return (input) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        var now = Date.now();
        this._runBranch(signal, tree, 0, now, { resolve, reject });
      });
    }
  }
   ...
   * @private
   */
  _runBranch (signal, tree, index, start, promise) {
    ...
    Array.isArray(currentBranch) ?
      this._runAsyncBranch(tree.branches, currentBranch, signal.input, promise) :
      this._runSyncBranch(tree.branches, currentBranch, signal.input, signal.name, promise);
  }
   ...
   * @private
   */
  _runAsyncBranch (branches, currentBranch, input, promise) {
    ...
    return Promise.all(promises)
      .then(() => {
        return this._runBranch(branches, index + 1, Date.now(), promise);
      })
      .catch(() => {
   ...
   * @private
   */
  _runSyncBranch (branches, currentBranch, input, signalName, promise) {
    try {
      ...
      if (result.path) {
        ...
        let result = this._runBranch(action.outputs[result.path], 0, promise);
  ...
  _createNextAsyncAction (actionFunc, signalName) {
    ...
    var next = this._createNextFunction(actionFunc, signalName, resolver);
    this._addOutputs(actionFunc, next);
    return { next, promise };
  }
