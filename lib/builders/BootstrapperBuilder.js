    ...
    return pfs.readFile(bootstrapperTemplatePath, {
      ...
      .then(file => {
        return Promise.all([
            ...
            self._generateRequires(signals),
            self._generateRequires(watchers)
          ])
  ...
  _generateRequires (entities) {
    ...
    Object
      ...
      .forEach(function (entityName) {
        ...
        entetiesRequires.push(util.format(ENTITY_FORMAT, entityName, requireExpression));
      });
    return Promise.resolve(entetiesRequires.join(','));
  };
